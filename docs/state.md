# State
_Last updated: 2026-04-28 0318_

## Current focus
Phone startup latency. Boot now stays on a much smaller critical path: the game only blocks on menu + early-game music, no longer waits on webfont readiness before showing the menu, and background-warms later gameplay stems with seek-alignment so the phase 3→4 swap should stay on-beat without front-loading the full music payload.

## What's working
- `src/game/scenes/BootScene.ts`: boot still shows the existing hologram loading presentation, but the mandatory hold is now a short flash (`450ms`) instead of a long wait, and the boot-to-menu handoff no longer blocks on font readiness.
...
- `src/game/systems/MusicSystem.ts`: later-loaded gameplay stems now start at the current gameplay reference stem's delayed seek position, so late loading should preserve beat alignment instead of reintroducing the old off-beat phase 3→4 crossfade.
...
- `npm.cmd run build`: passes after the startup optimization pass.
...
## Next actions
1. Cold-refresh on phone and compare menu-usable time before/after this pass.
...
## How to verify
1. `npm.cmd run build`
2. Hard-refresh the app on phone or mobile emulation and confirm the menu appears much sooner than before.
